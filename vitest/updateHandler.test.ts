/**
 * @file vitest/updateHandler.test.ts
 * @description This file contains the tests for the ShellyPlatform update handler.
 * @author Luca Liguori
 */
...
describe('ShellyPlatform', () => {
  ...
  for (const shellyId of shellies) {
    ...
    if (shellyId.id === 'shelly1pmg4-A085E3BD0544') {
      it(`should trigger a latching Press event for shelly1pmg4`, async () => {
        expect(device).not.toBeUndefined();
        if (!device) return;
        expect(endpoint).not.toBeUndefined();
        if (!endpoint) return;
        expect(device.getComponent('input:0')).not.toBeUndefined();
        expect(endpoint.getChildEndpointByOriginalId('input:0')).not.toBeUndefined();
        // shelly1pmg4 is on the inputLatchingList (see config above): a true state should trigger the 'Press' event
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'input:0', 'state', true);
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'input:0', 'state', false);
      });
    }
    ...
    if (shellyId.id === 'shellyhtg3-3030F9EC8468') {
      it(`should update charging Gen3 battery devices`, async () => {
        ...
        // Out-of-range voltage should be skipped
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'devicepower:0', 'battery', { V: 20, percent: 50 }, 'PowerSource');
      });
    }
    ...
    if (shellyId.id === 'shellytrv-60A423D0E032') {
      it(`should update thermostat for shellytrv`, async () => {
        ...
        // enabled is neither strictly true nor false: neither systemMode branch should fire
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'thermostat:0', 'target_t', { enabled: undefined, value: 50 }); // value also out of range (4-31)
        // is_valid false: the localTemperature update should be skipped
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'thermostat:0', 'tmp', { value: 21, is_valid: false });
      });

      it(`should not update thermostat when the device gen is not yet known`, async () => {
        expect(device).not.toBeUndefined();
        if (!device) return;
        expect(endpoint).not.toBeUndefined();
        if (!endpoint) return;
        const originalGen = device.gen;
        device.gen = 0; // Simulate a device whose generation has not been detected yet
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'thermostat:0', 'target_t', { enabled: true, value: 21 });
        device.gen = originalGen;
      });
    }
    ...
    if (shellyId.id === 'shellywalldisplay-00082261E102') {
      it(`should update thermostat for shellywalldisplay`, async () => {
        ...
        dataEndpoint.setAttribute = async (): Promise<boolean> => {
          return true;
        };
        ...
        // type is neither 'heating' nor 'cooling': no systemMode branch should fire
        device.getComponent('thermostat:0')?.setValue('type', 'auto');
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'thermostat:0', 'enable', true);
      });
    }
    ...
    if (shellyId.id === 'shellyswitch25-3494546BBF7E') {
      it(`should update roller for shellyswitch25`, async () => {
        ...
        // An out-of-range sys voltage should not be forwarded to the ElectricalPowerMeasurement cluster
        expect(device.getComponent('meter:0')).not.toBeUndefined();
        device.getComponent('sys')?.setValue('voltage', 5); // Below the minimum of 10
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'meter:0', 'power', 50);
        device.getComponent('sys')?.setValue('voltage', 230); // Restore a valid voltage for subsequent tests
      });
    }
    ...
    if (shellyId.id === 'shellyprorgbwwpm-2CBCBBA78F00') {
      it(`should fall back to the default color temperature range when ct_range is invalid`, async () => {
        expect(device).not.toBeUndefined();
        if (!device) return;
        expect(endpoint).not.toBeUndefined();
        if (!endpoint) return;
        const cct = device.getComponent('cct:0');
        expect(cct).not.toBeUndefined();
        if (!cct) return;
        expect(cct.getProperty('ct_range')).not.toBeUndefined();
        cct.setValue('ct_range', undefined);
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'cct:0', 'ct', 4000);
      });
    }
    ...
    if (shellyId.id === 'shellyplug-s-C38EAB') {
      it(`should calculate current from a component's own power and voltage`, async () => {
        expect(device).not.toBeUndefined();
        if (!device) return;
        expect(endpoint).not.toBeUndefined();
        if (!endpoint) return;
        const meter = device.getComponent('meter:0');
        expect(meter).not.toBeUndefined();
        if (!meter) return;
        // shellyplug-s meters do not report their own voltage: add it directly to exercise the fallback calculation
        meter.addProperty(new ShellyProperty(meter, 'voltage', 230));
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'meter:0', 'power', 50);
      });
    }

    if (shellyId.id === 'shellyplus1pm-441793D69718') {
      it(`should update current for a component without act_power`, async () => {
        expect(device).not.toBeUndefined();
        if (!device) return;
        expect(endpoint).not.toBeUndefined();
        if (!endpoint) return;
        expect(device.getComponent('switch:0')).not.toBeUndefined();
        // shellyplus1pm reports 'apower', not 'act_power'
        expect(device.getComponent('switch:0')?.hasProperty('act_power')).toBe(false);
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'switch:0', 'current', 1);

        // No real mock device reports 'current' without any of act_power/apower/power: build a synthetic
        // PowerMeter component to exercise the fallback power-from-current-and-voltage calculation.
        const synthetic = new ShellyComponent(device, 'synthetic:0', 'PowerMeter', { current: 1 });
        device.addComponent(synthetic);
        shellyUpdateHandler(shellyPlatform, endpoint, device, 'synthetic:0', 'current', 1, 'PowerSource');
      });
    }
      ...
      */
      ...
      for (const component of device.components) {
        for (const property of component.properties) {
          ...
          dataEndpoint ??= endpoint.getChildEndpointById(component.id);
          dataEndpoint ??= endpoint.getChildEndpointById(component.id.replace(':', ''));
          if (!dataEndpoint) {
