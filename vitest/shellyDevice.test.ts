/**
 * @file vitest/shellyDevice.test.ts
 * @description This file contains the tests for the ShellyDevice class.
 * @author Luca Liguori
 */

const NAME = 'ShellyDevice';
...
import type { BTHomeComponent, ShellyDataType } from '../src/shellyTypes.js';
...
let fetchSpy = vi.mocked(shellyFetch);
let createSpy = vi.spyOn(ShellyDevice, 'create');

describe('Shelly devices test', () => {
  // Mock functions for event handlers
  const handleOnline = vi.fn<() => void>().mockImplementation(() => {});

  const handleOffline = vi.fn<() => void>().mockImplementation(() => {});

  const handleAwake = vi.fn<() => void>().mockImplementation(() => {});

  const handleUpdate = vi.fn<(id: string, key: string, value: ShellyDataType) => void>().mockImplementation((id: string, key: string, value: ShellyDataType) => {});
  ...
  afterEach(() => {
    ...
    fetchSpy = vi.mocked(shellyFetch);
    createSpy.mockRestore();
    createSpy = vi.spyOn(ShellyDevice, 'create');
  });
  ...
  test('onEvent should handle bthomedevice event', async () => {
    ...
    const handleBthomeDeviceEvent = vi.fn();
  ...
  test('onEvent should handle bthomesensor event', async () => {
    ...
    const handleBthomeSensorEvent = vi.fn();
  ...
  test('onEvent should handle regular component event', async () => {
    ...
    const componentEventHandler = vi.fn();
    if (!component) return;
  ...
  test('onUpdate should handle bthomedevice update', async () => {
    ...
    const handleBthomeDeviceUpdate = vi.fn();
  ...
  test('onUpdate should handle bthomesensor update', async () => {
    ...
    const handleBthomeSensorUpdate = vi.fn();
  ...
  test('gen 2+ device should reconnect to websocket', async () => {
    ...
    vi.advanceTimersByTime(60 * 1000); // Simulate time passing for reconnection
  ...
  test('gen 2+ device should respond to awake', async () => {
    ...
    device.cached = true;
    device.gen = 1;
    createSpy.mockImplementationOnce(async () => ({ gen: 1, saveDevicePayloads: vi.fn(), destroy: vi.fn() }) as any);
    device.emit('awake');
  ...
  test('awake event should handle errors during awakening 2', async () => {
    ...
    const createSpy = vi.spyOn(ShellyDevice, 'create');
