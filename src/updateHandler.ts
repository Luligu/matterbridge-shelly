...
export function shellyUpdateHandler(
  ...
  endpoint ??= matterbridgeDevice.getChildEndpointById(endpointName ?? component);
  endpoint ??= matterbridgeDevice.getChildEndpointById(component.replace(':', ''));
  if (!endpoint) {
  ...
  if ((isLightComponent(shellyComponent) || isSwitchComponent(shellyComponent)) && property === 'state' && isValidBoolean(value)) {
    void endpoint.setAttribute(OnOff.id, 'onOff', value, shellyDevice.log);
  }
  // Update brightness
  if (isLightComponent(shellyComponent) && (property === 'gain' || property === 'brightness') && isValidNumber(value, 0, 100)) {
    void endpoint.setAttribute(LevelControl.id, 'currentLevel', Math.max(Math.min(Math.round((value / 100) * 254), 254), 1), shellyDevice.log);
  }
  // Update color gen 1
  if (isLightComponent(shellyComponent) && ['red', 'green', 'blue'].includes(property) && isValidNumber(value, 0, 255)) {
    ...
    shellyDevice.colorUpdateTimeout = setTimeout(() => {
      ...
      /* v8 ignore next cause hue is always clamped to 0-254 above, so it is always a valid number */
      if (isValidNumber(hue, 0, 254)) void endpoint?.setAttribute(ColorControl.id, 'currentHue', hue, shellyDevice.log);
      /* v8 ignore next cause saturation is always clamped to 0-254 above, so it is always a valid number */
      if (isValidNumber(saturation, 0, 254)) void endpoint?.setAttribute(ColorControl.id, 'currentSaturation', saturation, shellyDevice.log);
      void endpoint?.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.CurrentHueAndCurrentSaturation, shellyDevice.log);
  ...
  if (isLightComponent(shellyComponent) && property === 'temp' && isValidNumber(value, 2700, 6500)) {
    ...
    void endpoint.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.ColorTemperatureMireds, shellyDevice.log);
    void endpoint.setAttribute(ColorControl.id, 'enhancedColorMode', ColorControl.EnhancedColorMode.ColorTemperatureMireds, shellyDevice.log);
    void endpoint.setAttribute(ColorControl.id, 'colorTemperatureMireds', matterTemp, shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Input' && property === 'state' && isValidBoolean(value)) {
    if (platform.config.inputContactList?.includes(shellyDevice.id)) {
      void endpoint.setAttribute(BooleanState.id, 'stateValue', value, shellyDevice.log);
    }
    if (platform.config.inputMomentaryList?.includes(shellyDevice.id) && value) {
      void endpoint.triggerSwitchEvent('Single', shellyDevice.log);
    }
    if (platform.config.inputLatchingList?.includes(shellyDevice.id)) {
      void endpoint.triggerSwitchEvent(value ? 'Press' : 'Release', shellyDevice.log);
    }
  }
  ...
  if (shellyComponent.name === 'Input' && property === 'event_cnt' && isValidNumber(value) && shellyComponent.hasProperty('event')) {
    /* v8 ignore next cause a Gen 1 Input endpoint is only ever created when the device is on the inputMomentaryList (see module.ts), so this check is always true here */
    if (platform.config.inputMomentaryList?.includes(shellyDevice.id)) {
      ...
      if (event === 'SS') {
        void endpoint.triggerSwitchEvent('Double', shellyDevice.log);
      }
      if (event === 'L') {
        void endpoint.triggerSwitchEvent('Long', shellyDevice.log);
      }
    }
  }
  // Update for Battery
  if (shellyComponent.name === 'Battery' && property === 'level' && isValidNumber(value, 0, 100)) {
    void endpoint.setAttribute(PowerSource.id, 'batPercentRemaining', Math.min(Math.max(value * 2, 0), 200), shellyDevice.log);
    if (value < 10) void endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Critical, shellyDevice.log);
    else if (value < 20) void endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Warning, shellyDevice.log);
    else void endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Ok, shellyDevice.log);
  }
  if (shellyComponent.name === 'Battery' && property === 'voltage' && isValidNumber(value, 0)) {
    void endpoint.setAttribute(PowerSource.id, 'batVoltage', value * 1000, shellyDevice.log);
  }
  if (shellyComponent.name === 'Battery' && property === 'charging' && isValidNumber(value)) {
    void endpoint.setAttribute(PowerSource.id, 'batChargeState', value ? PowerSource.BatChargeState.IsCharging : PowerSource.BatChargeState.IsNotCharging, matterbridgeDevice.log);
  }
  ...
  if (shellyComponent.name === 'Devicepower' && property === 'battery' && isValidObject(value, 2)) {
    ...
    if (isValidNumber(battery.V, 0, 12) && isValidNumber(battery.percent, 0, 100)) {
      void endpoint.setAttribute(PowerSource.id, 'batPercentRemaining', battery.percent * 2, shellyDevice.log);
      if (battery.percent < 10) void endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Critical, shellyDevice.log);
      else if (battery.percent < 20) void endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Warning, shellyDevice.log);
      else void endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Ok, shellyDevice.log);
      void endpoint.setAttribute(PowerSource.id, 'batVoltage', battery.V * 1000, shellyDevice.log);
    }
  }

  // Update for Motion
  if (shellyComponent.name === 'Sensor' && property === 'motion' && isValidBoolean(value)) {
    void endpoint.setAttribute(OccupancySensing.id, 'occupancy', { occupied: value }, shellyDevice.log);
  }
  // Update for Contact
  ...
  if (shellyComponent.name === 'Flood' && property === 'flood' && isValidBoolean(value)) {
    // void endpoint.setAttribute(BooleanState.id, 'stateValue', !value, shellyDevice.log);
    void endpoint.setAttribute(BooleanState.id, 'stateValue', value, shellyDevice.log); // Water Leak Detector: true = leak, false = no leak
  }
  ...
  if (shellyComponent.name === 'Gas' && property === 'alarm_state' && isValidString(value)) {
    void endpoint.setAttribute(BooleanState.id, 'stateValue', value === 'none', shellyDevice.log);
  }
  // Update for Smoke
  if (shellyComponent.name === 'Smoke' && property === 'alarm' && isValidBoolean(value)) {
    // void endpoint.setAttribute(BooleanState.id, 'stateValue', !value, shellyDevice.log);
    void endpoint.setAttribute(SmokeCoAlarm.id, 'smokeState', value ? SmokeCoAlarm.AlarmState.Critical : SmokeCoAlarm.AlarmState.Normal, shellyDevice.log);
  }
  // Update for Lux
  if (shellyComponent.name === 'Lux' && property === 'value' && isValidNumber(value, 0)) {
    ...
    void endpoint.setAttribute(IlluminanceMeasurement.id, 'measuredValue', matterLux, shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Temperature' && (property === 'value' || property === 'tC') && isValidNumber(value, -100, 100)) {
    void endpoint.setAttribute(TemperatureMeasurement.id, 'measuredValue', Math.round(value * 100), shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Humidity' && (property === 'value' || property === 'rh') && isValidNumber(value, 0, 100)) {
    void endpoint.setAttribute(RelativeHumidityMeasurement.id, 'measuredValue', Math.round(value * 100), shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Vibration' && property === 'vibration' && isValidBoolean(value)) {
    if (value) void endpoint.triggerSwitchEvent('Single', shellyDevice.log);
  }
  if (shellyDevice.gen === 1) {
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'target_t' && isValidObject(value)) {
      ...
      // oxlint-disable-next-line typescript/no-unnecessary-boolean-literal-compare -- target.enabled is only type-asserted, not runtime-validated, so it may not actually be a boolean
      if (target.enabled === false) void endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Off, shellyDevice.log);
      // oxlint-disable-next-line typescript/no-unnecessary-boolean-literal-compare -- target.enabled is only type-asserted, not runtime-validated, so it may not actually be a boolean
      else if (target.enabled === true) void endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Heat, shellyDevice.log);
      if (isValidNumber(target.value, 4, 31)) void endpoint.setAttribute(Thermostat.id, 'occupiedHeatingSetpoint', target.value * 100, shellyDevice.log);
    }
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'tmp' && isValidObject(value)) {
      ...
      if (isValidNumber(current.value, -55, 125) && current.is_valid) void endpoint.setAttribute(Thermostat.id, 'localTemperature', current.value * 100, shellyDevice.log);
    }
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'enable' && isValidBoolean(value)) {
      if (!value) void endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Off, shellyDevice.log);
      else if (value && shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'heating')
        void endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Heat, shellyDevice.log);
      else if (value && shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'cooling')
        void endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Cool, shellyDevice.log);
    }
    // Update for Thermostat current_C (Gen2+)
    if (shellyComponent.name === 'Thermostat' && property === 'current_C' && isValidNumber(value, -100, 100)) {
      void endpoint.setAttribute(Thermostat.id, 'localTemperature', value * 100, shellyDevice.log);
    }
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'target_C' && isValidNumber(value, -100, 100)) {
      if (shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'heating')
        void endpoint.setAttribute(Thermostat.id, 'occupiedHeatingSetpoint', value * 100, shellyDevice.log);
      if (shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'cooling')
        void endpoint.setAttribute(Thermostat.id, 'occupiedCoolingSetpoint', value * 100, shellyDevice.log);
    }
  }
  // Update cover
  if (shellyComponent.name === 'Cover' || shellyComponent.name === 'Roller') {
    ...
    if (property === 'state' && isValidString(value, 4)) {
      ...
      if ((shellyDevice.gen === 1 && value === 'stop') || (shellyDevice.gen > 1 && value === 'stopped')) {
        ...
        void endpoint.setAttribute(WindowCovering.id, 'operationalStatus', { global: status, lift: status, tilt: status }, shellyDevice.log);
        shellyDevice.coverUpdateTimeout = setTimeout(() => {
          ...
          const current = endpoint?.getAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', shellyDevice.log);
          /* v8 ignore next cause is just a safety check, it should never happen that current position is not valid */
          ...
          void endpoint?.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', current, shellyDevice.log);
        }, shellyDevice.coverUpdateTimeoutMs).unref();
      }
      // Gen 2 devices send open for fully open
      if (shellyDevice.gen > 1 && value === 'open') {
        void endpoint.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', 0, shellyDevice.log);
        void endpoint.setAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', 0, shellyDevice.log);
        const status = WindowCovering.MovementStatus.Stopped;
        void endpoint.setAttribute(WindowCovering.id, 'operationalStatus', { global: status, lift: status, tilt: status }, shellyDevice.log);
      }
      // Gen 2 devices send closed for fully closed
      if (shellyDevice.gen > 1 && value === 'closed') {
        void endpoint.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', 10000, shellyDevice.log);
        void endpoint.setAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', 10000, shellyDevice.log);
        const status = WindowCovering.MovementStatus.Stopped;
        void endpoint.setAttribute(WindowCovering.id, 'operationalStatus', { global: status, lift: status, tilt: status }, shellyDevice.log);
      }
      ...
      void endpoint.setAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', matterPos, shellyDevice.log);
  ...
  if (['Light', 'Rgb', 'Relay', 'Switch', 'Cover', 'Roller', 'PowerMeter'].includes(shellyComponent.name)) {
    /* v8 ignore next cause is tested elsewhere */
    ...
    if (shellyDevice.profile === 'triphase' && shellyComponent.id === 'em:0') {
      ...
      if (property.startsWith('a_') || property.startsWith('b_') || property.startsWith('c_')) {
        if (property.startsWith('a_')) endpoint = matterbridgeDevice.getChildEndpointById('em:1');
        if (property.startsWith('b_')) endpoint = matterbridgeDevice.getChildEndpointById('em:2');
        if (property.startsWith('c_')) endpoint = matterbridgeDevice.getChildEndpointById('em:3');
        /* v8 ignore next cause is just a safety check, it should never happen that endpoint is not found */
        if (!endpoint) {
    ...
    if ((property === 'power' || property === 'apower' || property === 'act_power') && isValidNumber(value, 0)) {
      ...
      void endpoint.setAttribute(ElectricalPowerMeasurement.id, 'activePower', Math.round(power * 1000), shellyDevice.log);
      ...
      if (property === 'power' && shellyDevice.hasComponent('sys') && shellyDevice.getComponent('sys')?.hasProperty('voltage')) {
        ...
        if (isValidNumber(voltage, 10)) {
          void endpoint.setAttribute(ElectricalPowerMeasurement.id, 'voltage', Math.round(voltage * 1000), shellyDevice.log);
    ...
    if (property === 'voltage' && isValidNumber(value, 0)) {
      void endpoint.setAttribute(ElectricalPowerMeasurement.id, 'voltage', Math.round(value * 1000), shellyDevice.log);
    }
    if (property === 'current' && isValidNumber(value, 0)) {
      void endpoint.setAttribute(ElectricalPowerMeasurement.id, 'activeCurrent', Math.round(value * 1000), shellyDevice.log);
      ...
      /* oxlint-disable-next-line typescript/no-unsafe-type-assertion */ /* v8 ignore next */
      const voltage = shellyComponent.hasProperty('voltage') ? (shellyComponent.getValue('voltage') as number) : undefined;
      /* v8 ignore next */
