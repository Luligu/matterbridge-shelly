...
interface ResponseErrorMessage {
  ...
  nonce: number | string; // legacy: numeric, firmware 2.0.0+: base64 string
  nc?: number; // present only in legacy firmware (prior to 2.0.0); removed in firmware 2.0.0+
...
export class WsClient extends EventEmitter<WsClientEvent> {
  ...
  private requestFrameWithAuth: RequestFrameWithAuth = {
    ...
    method: 'Shelly.GetStatus',
    params: {},
    auth: { realm: '', username: 'admin', nonce: 0, cnonce: 0, nc: '00000001', response: '', algorithm: 'SHA-256' },
  };
