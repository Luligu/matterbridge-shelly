...
import { type Semtag, VendorId } from 'matterbridge/matter/types';
import { type NodeStorage, NodeStorageManager } from 'matterbridge/storage';
import {
  getErrorMessage,
  hslColorToRgbColor,
  isValidArray,
...
import { shellyCoverCommandHandler, shellyIdentifyCommandHandler, shellyLightCommandHandler, shellySwitchCommandHandler } from './commandHandlers.js';
...
import { shellyUpdateHandler } from './updateHandler.js';
...
export class ShellyPlatform extends MatterbridgeDynamicPlatform {
  ...
  constructor(
    ...
    this.shelly.on('discovered', async (discoveredDevice: DiscoveredDevice) => {
      ...
      if (
        (this.firstRun || !config.expertMode) &&
    ...
    this.shelly.on('add', async (device: ShellyDevice) => {
        ...
        if (device.bthomeDevices.size && device.bthomeSensors.size) {
          ...
          device.on('bthomesensor_update', (addr: string, sensorName: string, sensorIndex: number, value: ShellyDataType) => {
            ...
            if (blu && sensorName === 'Battery' && isValidNumber(value, 0, 100)) {
              void blu.setAttribute(PowerSource.id, 'batPercentRemaining', value * 2, blu.log);
              if (value < 10) void blu.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Critical, blu.log);
              else if (value < 20) void blu.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Warning, blu.log);
              else void blu.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Ok, blu.log);
            }
            if (blu && sensorName === 'Temperature' && isValidNumber(value, -100, 100)) {
              if (bthomeDevice.model === 'Shelly BLU Trv' && sensorIndex === 0) void blu.setAttribute(Thermostat.id, 'occupiedHeatingSetpoint', value * 100, blu.log);
              else if (bthomeDevice.model === 'Shelly BLU Trv' && sensorIndex === 1) void blu.setAttribute(Thermostat.id, 'localTemperature', value * 100, blu.log);
              else {
                const child = blu.getChildEndpointById('Temperature');
                void child?.setAttribute(TemperatureMeasurement.id, 'measuredValue', value * 100, blu.log);
              }
            }
            if (blu && sensorName === 'Humidity' && isValidNumber(value, 0, 100)) {
              const child = blu.getChildEndpointById('Humidity');
              void child?.setAttribute(RelativeHumidityMeasurement.id, 'measuredValue', value * 100, blu.log);
            }
            if (blu && sensorName === 'Illuminance' && isValidNumber(value, 0, 10000) && this.validateEntity(bthomeDevice.addr, 'Illuminance')) {
              const child = blu.getChildEndpointById('Illuminance');
              const matterLux = Math.round(Math.max(Math.min(10000 * Math.log10(value), 0xfffe), 0));
              void child?.setAttribute(IlluminanceMeasurement.id, 'measuredValue', matterLux, blu.log);
            }
            if (blu && sensorName === 'Motion' && isValidBoolean(value)) {
              const child = blu.getChildEndpointById('Motion');
              void child?.setAttribute(OccupancySensing.id, 'occupancy', { occupied: value }, blu.log);
            }
            if (blu && sensorName === 'Contact' && isValidBoolean(value)) {
              const child = blu.getChildEndpointById('Contact');
              void child?.setAttribute(BooleanState.id, 'stateValue', !value, blu.log);
            }
          });
          ...
          device.on('bthomedevice_event', (addr: string, event: ShellyEvent) => {
            ...
            if (event.event === 'config_changed') {
              ...
              device
                ...
                .then(() => {
                  ...
                  void device.saveDevicePayloads(this.shelly.dataPath);
                  return;
                })
                .catch((error: unknown) => {
                  device.log.error(
                    `Shelly device ${idn}${device.name}${rs}${er} id ${hk}${device.id}${er} host ${zb}${device.host}${er} failed to fetch update: ${CYAN}${getErrorMessage(error)}${er}`,
                  );
                });
            }
            if (['single_push', 'double_push', 'long_push'].includes(event.event)) {
              ...
              if (bthomeDevice.model === 'Shelly BLU RC Button 4' || bthomeDevice.model === 'Shelly BLU RC Button 4 ZB') {
                buttonEndpoint = blu.getChildEndpointById('Button' + event.idx);
                ...
                buttonEndpoint = blu.getChildEndpointById('Button');
              }
              if (!buttonEndpoint) {
                if (
                  [
              ...
              if (event.event === 'single_push') {
                void buttonEndpoint?.triggerSwitchEvent('Single', blu.log);
              } else if (event.event === 'double_push') {
                void buttonEndpoint?.triggerSwitchEvent('Double', blu.log);
              } else if (event.event === 'long_push') {
                void buttonEndpoint?.triggerSwitchEvent('Long', blu.log);
              }
            }
          });
          ...
          device.on('bthomesensor_event', (addr: string, sensorName: string, sensorIndex: number, event: ShellyEvent) => {
            ...
            let buttonEndpoint: MatterbridgeEndpoint | undefined;
            if (bthomeDevice.model === 'Shelly BLU RC Button 4' || bthomeDevice.model === 'Shelly BLU RC Button 4 ZB') {
              buttonEndpoint = blu.getChildEndpointById('Button' + sensorIndex);
            } else if (bthomeDevice.model === 'Shelly BLU Wall Switch 4' || bthomeDevice.model === 'Shelly BLU Wall Switch 4 ZB') {
              buttonEndpoint = blu.getChildEndpointById('Button' + sensorIndex);
            } else if (bthomeDevice.model === 'Shelly BLU Button1' || bthomeDevice.model === 'Shelly BLU Button Tough 1 ZB') {
              buttonEndpoint = blu;
      ...
      if (this.validateEntity(device.id, 'PowerSource')) {
        ...
        if (batteryComponent) {
          ...
          if (devicepowerComponent.hasProperty('battery') && isValidObject(devicepowerComponent.getValue('battery'), 2)) {
            ...
            if (isValidBoolean(external.present) && external.present) {
      ...
      if (names.includes('Light') || names.includes('Rgb')) {
        void mbDevice.addFixedLabel('composed', 'Light');
      } else if (names.includes('Switch') || names.includes('Relay')) {
        void mbDevice.addFixedLabel('composed', 'Switch');
      } else if (names.includes('Cover') || names.includes('Roller')) {
        void mbDevice.addFixedLabel('composed', 'Cover');
      } else if (names.includes('PowerMeter')) {
        void mbDevice.addFixedLabel('composed', 'PowerMeter');
      } else if (names.includes('Input')) {
        void mbDevice.addFixedLabel('composed', 'Input');
      } else if (names.includes('Blugw')) {
        void mbDevice.addFixedLabel('composed', 'BLU Gateway');
      } else {
      ...
      for (const [key, component] of device) {
        ...
        if (component.name === 'Ble') {
          ...
          child.addCommandHandler('identify', ({ request }) => {
            shellyIdentifyCommandHandler(child, component, request);
          });
          child.addCommandHandler('on', () => {
            shellyLightCommandHandler(child, component, 'On');
          });
          child.addCommandHandler('off', () => {
            shellyLightCommandHandler(child, component, 'Off');
          });
          child.addCommandHandler('toggle', () => {
            shellyLightCommandHandler(child, component, 'Toggle');
          });
          child.addCommandHandler('moveToLevel', ({ request }) => {
            shellyLightCommandHandler(child, component, 'Level', request.level);
          });
          child.addCommandHandler('moveToLevelWithOnOff', ({ request }) => {
            shellyLightCommandHandler(child, component, 'Level', request.level);
          });
          child.addCommandHandler('moveToHue', ({ request }) => {
            // child.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.CurrentHueAndCurrentSaturation, child.log);
            const saturation = child.getAttribute(ColorControl.id, 'currentSaturation', child.log);
          ...
          child.addCommandHandler('moveToSaturation', ({ request }) => {
            // child.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.CurrentHueAndCurrentSaturation, child.log);
            const hue = child.getAttribute(ColorControl.id, 'currentHue', child.log);
          ...
          child.addCommandHandler('moveToHueAndSaturation', ({ request }) => {
            // child.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.CurrentHueAndCurrentSaturation, child.log);
            const rgb = hslColorToRgbColor((request.hue / 254) * 360, (request.saturation / 254) * 100, 50);
            shellyLightCommandHandler(child, component, 'ColorRGB', undefined, { r: rgb.r, g: rgb.g, b: rgb.b });
          });
          child.addCommandHandler('moveToColor', ({ request }) => {
            // child.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.CurrentXAndCurrentY, child.log);
            const rgb = xyColorToRgbColor(request.colorX / 65536, request.colorY / 65536);
            shellyLightCommandHandler(child, component, 'ColorRGB', undefined, { r: rgb.r, g: rgb.g, b: rgb.b });
          });
          child.addCommandHandler('moveToColorTemperature', ({ request }) => {
            // child.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.ColorTemperatureMireds, child.log);
          ...
          let deviceType = onOffPlugInUnit;
          if (config.switchList?.includes(device.id)) deviceType = onOffLightSwitch;
          if (config.lightList?.includes(device.id)) deviceType = onOffLight;
          ...
          const child = mbDevice.addChildDeviceType(
            key,
            this.hasElectricalMeasurements(device, component) && this.validateEntity(device.id, 'PowerMeter') ? [windowCovering, electricalSensor] : [windowCovering],
            tagList ? { tagList } : undefined,
            config.debug,
          );
          child.log.logName = `${device.name} ${key}`;
          ...
          child.addCommandHandler('upOrOpen', ({ attributes }) => {
            // child.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', 0, child.log);
            attributes.currentPositionLiftPercent100ths = 0;
            shellyCoverCommandHandler(child, component, 'Open', 0);
          });
          child.addCommandHandler('downOrClose', ({ attributes }) => {
            // child.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', 10000, child.log);
            attributes.currentPositionLiftPercent100ths = 10000;
            shellyCoverCommandHandler(child, component, 'Close', 10000);
          });
          child.addCommandHandler('stopMotion', () => {
            shellyCoverCommandHandler(child, component, 'Stop');
          });
          child.addCommandHandler('goToLiftPercentage', ({ request, attributes }) => {
            // child.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', request.liftPercent100thsValue, child.log);
          ...
          if (pmComponent) {
            ...
            const child = mbDevice.addChildDeviceTypeWithClusterServer(
              key,
              [electricalSensor],
              [ElectricalPowerMeasurement.id, ElectricalEnergyMeasurement.id],
              tagList ? { tagList } : undefined,
              config.debug,
            );
            child.log.logName = `${device.name} ${key}`;
          ...
          if (component.hasProperty('state') && config.inputContactList?.includes(device.id)) {
            const state = component.getValue('state') as boolean;
            if (isValidBoolean(state)) {
              const child = mbDevice.addChildDeviceType(key, [contactSensor], tagList ? { tagList } : undefined, config.debug);
              child.log.logName = `${device.name} ${key}`;
          ...
          } else if (component?.hasProperty('state') && config.inputMomentaryList?.includes(device.id)) {
            const state = component.getValue('state') as boolean;
            if (isValidBoolean(state)) {
              const child = mbDevice.addChildDeviceType(key, [genericSwitch], tagList ? { tagList } : undefined, config.debug);
              child.log.logName = `${device.name} ${key}`;
          ...
          } else if (component?.hasProperty('state') && config.inputLatchingList?.includes(device.id)) {
            const state = component.getValue('state') as boolean;
            if (isValidBoolean(state)) {
              const child = mbDevice.addChildDeviceType(key, [genericSwitch], tagList ? { tagList } : undefined, config.debug);
              child.log.logName = `${device.name} ${key}`;
          ...
          } else if (component?.hasProperty('event') && config.inputMomentaryList?.includes(device.id)) {
          ...
          if (
            ...
            config.inputMomentaryList?.includes(device.id)
          ) {
            ...
            component.on('event', (component: string, event: string) => {
              if (isValidString(component, 7) && isValidString(event, 9, 11) && device.getComponent(component)) {
                ...
                const endpoint = mbDevice.getChildEndpointById(component);
                if (!endpoint) {
                  device.log.error(`getChildEndpointById(${component}) for device ${idn}${device.id}${rs} failed`);
                  return;
                }
                if (event === 'single_push') void endpoint.triggerSwitchEvent('Single', device.log);
                if (event === 'double_push') void endpoint.triggerSwitchEvent('Double', device.log);
                if (event === 'long_push') void endpoint.triggerSwitchEvent('Long', device.log);
              }
            });
          }
          ...
          if (sensorComponent?.hasProperty('contact_open')) {
            const child = mbDevice.addChildDeviceType(key, [contactSensor], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (sensorComponent?.hasProperty('motion')) {
            const child = mbDevice.addChildDeviceType(key, [occupancySensor], undefined, config.debug);
          ...
          if (vibrationComponent?.hasProperty('vibration') && isValidBoolean(vibrationComponent.getValue('vibration'))) {
            const child = mbDevice.addChildDeviceType(key, [genericSwitch], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
            child.createDefaultSwitchClusterServer();
            child.addRequiredClusterServers();
            // Add event handler
          ...
          if (tempComponent?.hasProperty('value') && isValidNumber(tempComponent.getValue('value'))) {
            const child = mbDevice.addChildDeviceTypeWithClusterServer(key, [temperatureSensor], [], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (humidityComponent?.hasProperty('value') && isValidNumber(humidityComponent.getValue('value'), 0, 100)) {
            const child = mbDevice.addChildDeviceTypeWithClusterServer(key, [humiditySensor], [], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (illuminanceComponent?.hasProperty('lux') && isValidNumber(illuminanceComponent.getValue('lux'), 0, 10000)) {
            const child = mbDevice.addChildDeviceTypeWithClusterServer(key, [lightSensor], [], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (
            ...
            const child = mbDevice.addChildDeviceType(key, [thermostat], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
            child.createDefaultIdentifyClusterServer();
          ...
          if (
            ...
            if (thermostatComponent.getValue('type') === 'heating') {
              child = mbDevice.addChildDeviceType(key, [thermostat], undefined, config.debug);
              child.log.logName = `${device.name} ${key}`;
              child.createDefaultIdentifyClusterServer();
          ...
          if (floodComponent?.hasProperty('flood') && isValidBoolean(floodComponent.getValue('flood'))) {
            ...
            const child = mbDevice.addChildDeviceType(key, [waterLeakDetector], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (smokeComponent?.hasProperty('alarm') && isValidBoolean(smokeComponent.getValue('alarm'))) {
            const child = mbDevice.addChildDeviceType(key, [smokeCoAlarm], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (luxComponent?.hasProperty('value') && isValidNumber(luxComponent.getValue('value'), 0)) {
            const child = mbDevice.addChildDeviceType(key, [lightSensor], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
          ...
          if (blugwComponent?.hasProperty('sys_led_enable') && isValidBoolean(blugwComponent.getValue('sys_led_enable'))) {
            const child = mbDevice.addChildDeviceType(key, [modeSelect], undefined, config.debug);
            child.log.logName = `${device.name} ${key}`;
      ...
      try {
        ...
        device.on('online', () => {
          ...
          if (mbDevice.maybeNumber) {
            void mbDevice.setAttribute('BridgedDeviceBasicInformation', 'reachable', true, mbDevice.log);
            void mbDevice.triggerEvent(BridgedDeviceBasicInformation.id, 'reachableChanged', { reachableNewValue: true }, mbDevice.log);
          }
        });
        device.on('offline', () => {
          mbDevice.log.warn(`Device ${hk}${device.id}${wr} host ${zb}${device.host}${wr} is offline`);
          if (mbDevice.maybeNumber) {
            void mbDevice.setAttribute('BridgedDeviceBasicInformation', 'reachable', false, mbDevice.log);
            void mbDevice.triggerEvent(BridgedDeviceBasicInformation.id, 'reachableChanged', { reachableNewValue: false }, mbDevice.log);
          }
        });
      } catch (error) {
        this.log.error(`Shelly device ${hk}${device.id}${er} host ${zb}${device.host}${er} failed to register with Matterbridge: ${getErrorMessage(error)}`);
      }
    });
  }
  ...
  override async onStart(reason?: string): Promise<void> {
    ...
    if (this.config.resetStorageDiscover) {
      ...
      for (const device of storedDevices) {
        ...
        try {
          ...
          this.log.debug(`****Failed to delete cache for device ${device.id} file ${fileName} error: ${getErrorMessage(error)}`);
        }
      }
    ...
    if (this.config.enableStorageDiscover) {
      ...
      for (const storedDevice of this.storedDevices.values()) {
        ...
        storedDevice.id = normalizeId(storedDevice.id).id;
    ...
    if (this.config.enableMdnsDiscover) {
      this.shelly.mdnsScanner.start(0, 10 * 60 * 1000, this.config.interfaceName as string | undefined, 'udp4', this.config.debugMdns);
    }
    ...
    for (const mbDevice of this.bridgedDevices.values()) {
      ...
      for (const childEndpoint of mbDevice.getChildEndpoints()) {
        ...
        if (label.startsWith('switch') || label.startsWith('relay') || label.startsWith('light') || label.startsWith('rgb')) {
          ...
          if (isValidBoolean(state)) {
            await childEndpoint.setAttribute(OnOff.id, 'onOff', state, shellyDevice.log);
          }
        }
        ...
        if (label.startsWith('light') || label.startsWith('rgb')) {
          ...
          if (isValidNumber(level, 1, 100)) {
            ...
            await childEndpoint.setAttribute(LevelControl.id, 'currentLevel', matterLevel, shellyDevice.log);
          }
          ...
          if (lightComponent.hasProperty('red') && lightComponent.hasProperty('green') && lightComponent.hasProperty('blue') && shellyDevice.profile !== 'white') {
            ...
            if (isValidNumber(red, 0, 255) && isValidNumber(green, 0, 255) && isValidNumber(blue, 0, 255)) {
              ...
              if (isValidNumber(hue, 0, 254)) await childEndpoint.setAttribute(ColorControl.id, 'currentHue', hue, shellyDevice.log);
              if (isValidNumber(saturation, 0, 254)) await childEndpoint.setAttribute(ColorControl.id, 'currentSaturation', saturation, shellyDevice.log);
              await childEndpoint.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.CurrentHueAndCurrentSaturation, shellyDevice.log);
            }
          }
          if (lightComponent.hasProperty('temp') && shellyDevice.profile !== 'color') {
            await childEndpoint.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.ColorTemperatureMireds, shellyDevice.log);
          }
        ...
        if (label.startsWith('thermostat')) {
          ...
          if (isValidNumber(target, 5, 35)) {
            if (thermostatComponent.getValue('type') === 'heating') {
              ...
              await childEndpoint.setAttribute(Thermostat.id, 'occupiedHeatingSetpoint', target * 100, shellyDevice.log);
            } else if (thermostatComponent.getValue('type') === 'cooling') {
              ...
              await childEndpoint.setAttribute(Thermostat.id, 'occupiedCoolingSetpoint', target * 100, shellyDevice.log);
            }
          }
        }
      ...
      if (shellyDevice.bthomeDevices.size > 0) {
        ...
        shellyDevice.bthomeDevices.forEach((bthomeDevice) => {
          ...
          shellyDevice.bthomeSensors.forEach((bthomeSensor) => {
  ...
  override async onAction(action: string, value?: string): Promise<void> {
    ...
    if (action === 'removeDevice' && value && isValidString(value)) {
      ...
      if (this.shelly.hasDevice(value)) {
        ...
        this.config.blackList.push(value);
      ...
      try {
        ...
        this.log.debug(`****Failed to delete cache for device ${value} file ${fileName} error: ${getErrorMessage(error)}`);
      }
      await this.clearDeviceSelect(value);
  ...
  private hasElectricalMeasurements(shelly: ShellyDevice, component: ShellyComponent): boolean {
  ...
  private addElectricalMeasurements(device: MatterbridgeEndpoint, endpoint: MatterbridgeEndpoint, shelly: ShellyDevice, component: ShellyComponent): void {
    // Check if PowerMetering is enabled
  ...
  private async addBluDevice(
    ...
    else if (bthomeDevice.model === 'Shelly BLU Trv') definition = [thermostat, bridgedNode, powerSource];
    ...
    if (definition) {
      ...
      if (bthomeDevice.model === 'Shelly BLU DoorWindow' || bthomeDevice.model === 'Shelly BLU DoorWindow ZB') {
        ...
        if (this.validateEntity(bthomeDevice.addr, 'Illuminance')) mbDevice.addChildDeviceTypeWithClusterServer('Illuminance', [lightSensor], [], undefined, this.config.debug);
        if (this.validateEntity(bthomeDevice.addr, 'Button')) mbDevice.addChildDeviceTypeWithClusterServer('Button', [genericSwitch], [], undefined, this.config.debug);
      } else if (bthomeDevice.model === 'Shelly BLU Button1' || bthomeDevice.model === 'Shelly BLU Button Tough 1 ZB') {
        mbDevice.createDefaultPowerSourceReplaceableBatteryClusterServer();
        ...
        mbDevice.createDefaultPowerSourceReplaceableBatteryClusterServer();
        void mbDevice.addFixedLabel('composed', 'Sensor');
        mbDevice.addChildDeviceTypeWithClusterServer('Temperature', [temperatureSensor], [], undefined, this.config.debug);
        mbDevice.addChildDeviceTypeWithClusterServer('Humidity', [humiditySensor], [], undefined, this.config.debug);
        if (this.validateEntity(bthomeDevice.addr, 'Button')) mbDevice.addChildDeviceTypeWithClusterServer('Button', [genericSwitch], [], undefined, this.config.debug);
      } else if (bthomeDevice.model === 'Shelly BLU RC Button 4' || bthomeDevice.model === 'Shelly BLU RC Button 4 ZB') {
        mbDevice.createDefaultPowerSourceReplaceableBatteryClusterServer();
        void mbDevice.addFixedLabel('composed', 'Input');
        mbDevice.addChildDeviceTypeWithClusterServer('Button0', [genericSwitch], [Switch.id], undefined, this.config.debug);
        mbDevice.addChildDeviceTypeWithClusterServer('Button1', [genericSwitch], [Switch.id], undefined, this.config.debug);
        mbDevice.addChildDeviceTypeWithClusterServer('Button2', [genericSwitch], [Switch.id], undefined, this.config.debug);
        mbDevice.addChildDeviceTypeWithClusterServer('Button3', [genericSwitch], [Switch.id], undefined, this.config.debug);
      } else if (bthomeDevice.model === 'Shelly BLU Wall Switch 4' || bthomeDevice.model === 'Shelly BLU Wall Switch 4 ZB') {
        mbDevice.createDefaultPowerSourceReplaceableBatteryClusterServer();
        void mbDevice.addFixedLabel('composed', 'Input');
        mbDevice.addChildDeviceTypeWithClusterServer('Button0', [genericSwitch], [Switch.id], undefined, this.config.debug);
            ...
            if (
              ...
              if (oldValue === Thermostat.SystemMode.Heat && newValue === Thermostat.SystemMode.Off) {
                ...
                gateway.thermostatSystemModeTimeout = setTimeout(() => {
                  void mbDevice.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Heat, mbDevice.log);
                }, 5000);
              }
            }
          },
      ...
      try {
        ...
        this.log.error(
          `Shelly device ${hk}${gateway.id}${er} host ${zb}${gateway.host}${er} failed to register BLU device ${idn}${bthomeDevice.name}${rs}${er}: ${getErrorMessage(error)}`,
        );
      }
    }
  }
