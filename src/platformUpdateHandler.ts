...
export function shellyUpdateHandler(
  ...
  if (!endpoint) endpoint = matterbridgeDevice.getChildEndpointById(endpointName ?? component);
  if (!endpoint) endpoint = matterbridgeDevice.getChildEndpointById(component.replace(':', ''));
  if (!endpoint) {
  ...
  if ((isLightComponent(shellyComponent) || isSwitchComponent(shellyComponent)) && property === 'state' && isValidBoolean(value)) {
    endpoint.setAttribute(OnOff.id, 'onOff', value, shellyDevice.log);
  }
  // Update brightness
  if (isLightComponent(shellyComponent) && (property === 'gain' || property === 'brightness') && isValidNumber(value, 0, 100)) {
    endpoint.setAttribute(LevelControl.id, 'currentLevel', Math.max(Math.min(Math.round((value / 100) * 254), 254), 1), shellyDevice.log);
  }
  // Update color gen 1
  ...
  if (isLightComponent(shellyComponent) && property === 'temp' && isValidNumber(value, 2700, 6500)) {
    ...
    endpoint.setAttribute(ColorControl.id, 'colorMode', ColorControl.ColorMode.ColorTemperatureMireds, shellyDevice.log);
    endpoint.setAttribute(ColorControl.id, 'enhancedColorMode', ColorControl.EnhancedColorMode.ColorTemperatureMireds, shellyDevice.log);
    endpoint.setAttribute(ColorControl.id, 'colorTemperatureMireds', matterTemp, shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Input' && property === 'state' && isValidBoolean(value)) {
    if (platform.config.inputContactList && (platform.config.inputContactList as string[]).includes(shellyDevice.id)) {
      endpoint.setAttribute(BooleanState.id, 'stateValue', value, shellyDevice.log);
    }
  ...
  if (shellyComponent.name === 'Battery' && property === 'level' && isValidNumber(value, 0, 100)) {
    endpoint.setAttribute(PowerSource.id, 'batPercentRemaining', Math.min(Math.max(value * 2, 0), 200), shellyDevice.log);
    if (value < 10) endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Critical, shellyDevice.log);
    else if (value < 20) endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Warning, shellyDevice.log);
    else endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Ok, shellyDevice.log);
  }
  if (shellyComponent.name === 'Battery' && property === 'voltage' && isValidNumber(value, 0)) {
    endpoint.setAttribute(PowerSource.id, 'batVoltage', value * 1000, shellyDevice.log);
  }
  if (shellyComponent.name === 'Battery' && property === 'charging' && isValidNumber(value)) {
    endpoint.setAttribute(PowerSource.id, 'batChargeState', value ? PowerSource.BatChargeState.IsCharging : PowerSource.BatChargeState.IsNotCharging, matterbridgeDevice.log);
  }
  ...
  if (shellyComponent.name === 'Devicepower' && property === 'battery' && isValidObject(value, 2)) {
    ...
    if (isValidNumber(battery.V, 0, 12) && isValidNumber(battery.percent, 0, 100)) {
      endpoint.setAttribute(PowerSource.id, 'batPercentRemaining', battery.percent * 2, shellyDevice.log);
      if (battery.percent < 10) endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Critical, shellyDevice.log);
      else if (battery.percent < 20) endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Warning, shellyDevice.log);
      else endpoint.setAttribute(PowerSource.id, 'batChargeLevel', PowerSource.BatChargeLevel.Ok, shellyDevice.log);
      endpoint.setAttribute(PowerSource.id, 'batVoltage', battery.V * 1000, shellyDevice.log);
    }
  }

  // Update for Motion
  if (shellyComponent.name === 'Sensor' && property === 'motion' && isValidBoolean(value)) {
    endpoint.setAttribute(OccupancySensing.id, 'occupancy', { occupied: value }, shellyDevice.log);
  }
  // Update for Contact
  ...
  if (shellyComponent.name === 'Flood' && property === 'flood' && isValidBoolean(value)) {
    // endpoint.setAttribute(BooleanState.id, 'stateValue', !value, shellyDevice.log);
    endpoint.setAttribute(BooleanState.id, 'stateValue', value, shellyDevice.log); // Water Leak Detector: true = leak, false = no leak
  }
  ...
  if (shellyComponent.name === 'Gas' && property === 'alarm_state' && isValidString(value)) {
    endpoint.setAttribute(BooleanState.id, 'stateValue', value === 'none', shellyDevice.log);
  }
  // Update for Smoke
  if (shellyComponent.name === 'Smoke' && property === 'alarm' && isValidBoolean(value)) {
    // endpoint.setAttribute(BooleanState.id, 'stateValue', !value, shellyDevice.log);
    endpoint.setAttribute(SmokeCoAlarm.id, 'smokeState', value ? SmokeCoAlarm.AlarmState.Critical : SmokeCoAlarm.AlarmState.Normal, shellyDevice.log);
  }
  // Update for Lux
  if (shellyComponent.name === 'Lux' && property === 'value' && isValidNumber(value, 0)) {
    ...
    endpoint.setAttribute(IlluminanceMeasurement.id, 'measuredValue', matterLux, shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Temperature' && (property === 'value' || property === 'tC') && isValidNumber(value, -100, +100)) {
    endpoint.setAttribute(TemperatureMeasurement.id, 'measuredValue', Math.round(value * 100), shellyDevice.log);
  }
  ...
  if (shellyComponent.name === 'Humidity' && (property === 'value' || property === 'rh') && isValidNumber(value, 0, 100)) {
    endpoint.setAttribute(RelativeHumidityMeasurement.id, 'measuredValue', Math.round(value * 100), shellyDevice.log);
  }
  ...
  if (shellyDevice.gen === 1) {
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'target_t' && isValidObject(value)) {
      ...
      if (target.enabled === false) endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Off, shellyDevice.log);
      else if (target.enabled === true) endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Heat, shellyDevice.log);
      if (isValidNumber(target.value, 4, 31)) endpoint.setAttribute(Thermostat.id, 'occupiedHeatingSetpoint', target.value * 100, shellyDevice.log);
    }
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'tmp' && isValidObject(value)) {
      ...
      if (isValidNumber(current.value, -55, +125) && current.is_valid === true) endpoint.setAttribute(Thermostat.id, 'localTemperature', current.value * 100, shellyDevice.log);
    }
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'enable' && isValidBoolean(value)) {
      if (value === false) endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Off, shellyDevice.log);
      else if (value === true && shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'heating')
        endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Heat, shellyDevice.log);
      else if (value === true && shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'cooling')
        endpoint.setAttribute(Thermostat.id, 'systemMode', Thermostat.SystemMode.Cool, shellyDevice.log);
    }
    // Update for Thermostat current_C (Gen2+)
    if (shellyComponent.name === 'Thermostat' && property === 'current_C' && isValidNumber(value, -100, +100)) {
      endpoint.setAttribute(Thermostat.id, 'localTemperature', value * 100, shellyDevice.log);
    }
    ...
    if (shellyComponent.name === 'Thermostat' && property === 'target_C' && isValidNumber(value, -100, +100)) {
      if (shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'heating')
        endpoint.setAttribute(Thermostat.id, 'occupiedHeatingSetpoint', value * 100, shellyDevice.log);
      if (shellyComponent.hasProperty('type') && shellyComponent.getValue('type') === 'cooling')
        endpoint.setAttribute(Thermostat.id, 'occupiedCoolingSetpoint', value * 100, shellyDevice.log);
    }
  }
  // Update cover
  if (shellyComponent.name === 'Cover' || shellyComponent.name === 'Roller') {
    ...
    if (property === 'state' && isValidString(value, 4)) {
      ...
      if ((shellyDevice.gen === 1 && value === 'stop') || (shellyDevice.gen > 1 && value === 'stopped')) {
        ...
        endpoint.setAttribute(WindowCovering.id, 'operationalStatus', { global: status, lift: status, tilt: status }, shellyDevice.log);
        shellyDevice.coverUpdateTimeout = setTimeout(() => {
          ...
          endpoint?.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', current, shellyDevice.log);
        }, shellyDevice.coverUpdateTimeoutMs).unref();
      }
      // Gen 2 devices send open for fully open
      if (shellyDevice.gen > 1 && value === 'open') {
        endpoint.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', 0, shellyDevice.log);
        endpoint.setAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', 0, shellyDevice.log);
        const status = WindowCovering.MovementStatus.Stopped;
        endpoint.setAttribute(WindowCovering.id, 'operationalStatus', { global: status, lift: status, tilt: status }, shellyDevice.log);
      }
      // Gen 2 devices send closed for fully closed
      if (shellyDevice.gen > 1 && value === 'closed') {
        endpoint.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', 10000, shellyDevice.log);
        endpoint.setAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', 10000, shellyDevice.log);
        const status = WindowCovering.MovementStatus.Stopped;
        endpoint.setAttribute(WindowCovering.id, 'operationalStatus', { global: status, lift: status, tilt: status }, shellyDevice.log);
      }
      ...
      endpoint.setAttribute(WindowCovering.id, 'currentPositionLiftPercent100ths', matterPos, shellyDevice.log);
    } else if (property === 'target_pos' && isValidNumber(value, 0, 100)) {
      const matterPos = 10000 - Math.min(Math.max(Math.round(value * 100), 0), 10000);
      endpoint.setAttribute(WindowCovering.id, 'targetPositionLiftPercent100ths', matterPos, shellyDevice.log);
    }
  ...
  if (['Light', 'Rgb', 'Relay', 'Switch', 'Cover', 'Roller', 'PowerMeter'].includes(shellyComponent.name)) {
    ...
    if ((property === 'power' || property === 'apower' || property === 'act_power') && isValidNumber(value, 0)) {
      ...
      endpoint.setAttribute(ElectricalPowerMeasurement.id, 'activePower', Math.round(power * 1000), shellyDevice.log);
      ...
      if (property === 'power' && shellyDevice.hasComponent('sys') && shellyDevice.getComponent('sys')?.hasProperty('voltage')) {
        ...
        if (isValidNumber(voltage, 10)) {
          endpoint.setAttribute(ElectricalPowerMeasurement.id, 'voltage', Math.round(voltage * 1000), shellyDevice.log);
          const current = Math.round((value / voltage) * 1000) / 1000;
          endpoint.setAttribute(ElectricalPowerMeasurement.id, 'activeCurrent', Math.round(current * 1000), shellyDevice.log);
        }
      }
    ...
    if (property === 'total' && isValidNumber(value, 0)) {
      ...
      endpoint.setAttribute(ElectricalEnergyMeasurement.id, 'cumulativeEnergyImported', { energy: Math.round(energy * 1000) }, shellyDevice.log);
    }
    ...
    if (property === 'total_act_ret_energy' && isValidNumber(value, 0)) {
      const energy = Math.round(value * 1000) / 1000;
      endpoint.setAttribute(ElectricalEnergyMeasurement.id, 'cumulativeEnergyExported', { energy: Math.round(energy * 1000) }, shellyDevice.log);
    }
    if (property === 'voltage' && isValidNumber(value, 0)) {
      endpoint.setAttribute(ElectricalPowerMeasurement.id, 'voltage', Math.round(value * 1000), shellyDevice.log);
    }
    if (property === 'current' && isValidNumber(value, 0)) {
      endpoint.setAttribute(ElectricalPowerMeasurement.id, 'activeCurrent', Math.round(value * 1000), shellyDevice.log);
